...
export const CardListing = (
  ...
  useEffect(() => {
    districtdatas.map((e) => {
      if (
        router.query.collegelist.toLowerCase().replace(/-colleges/g, "") ==
        e.english_name.toString().toLowerCase()
      ) {
  ...
  useEffect(() => {
    setMenuname(
      router.query.collegelist.replace(/_/g, " ").replace("and", "&")
    );
  }, [router.query.collegelist]);
  ...
  useEffect(() => {
    ...
    // filteroptions.degrees?.map((el,ind)=>
    // {
    //   if(el.degreeid == streamsid)
    //   {
    //     if(el.name == menuname)
    //     {
    //     }
  ...
  //     setAlldistrictinputsid((a) => [a, searchDistrict]);
  //   }
  //   if (activeStr === true) {
  ...
  //     delete router.query.cityId;
  //   }
  // }, [districtBread, router.query.collegelist, activeDeg, activeStr]);
    ...
    if (latlngValue.latitude != undefined) {
      ...
      if (callCity != undefined) {
        router.query.collegelist && !router.query.course
          ? router.push(
              `/${router.query.collegelist}?districtId=${callCity}&cityId=${callArea}`
            )
          : router.query.collegelist && router.query.course
          ? router.push(
              `/${router.query.collegelist}?course= ${router.query.course}&districtId=${callCity}&cityId=${callArea}`
            )
          : null;
      }
    }
  ...
  useEffect(() => {
    if (
      ...
      pageNum != 0 ||
      menuSID != 0 ||
      router.asPath == "/tamilnadu-colleges"
    ) {
      axios
        .get(
          router.query.collegelist &&
            ...
            : router.query.collegelist
                .toLowerCase()
                .replace(/-colleges/g, "") == districtBread &&
              !router.query.course &&
              // activeDeg === false &&
              // activeStr === false &&
              sortRatingLow === false &&
            ...
            ? // &&
              // sortPopular === true
              `${CollegesAPI}?districtId=${searchDistrict}${decodeURIComponent(
                pathAlter.replace(/[?]/g, "&")
              ).replace(/_/g, ",")}&page=${pageNum}`
            : router.query.collegelist
                .toLowerCase()
            ...
            ? // &&
              // sortPopular === true
              `${CollegesAPI}?districtId=${searchDistrict}&sort=ranking${decodeURIComponent(
                pathAlter.replace(/[?]/g, "&")
              ).replace(/_/g, ",")}&page=${pageNum}`
            : router.query.collegelist
                .toLowerCase()
            ...
            ? // &&
              // sortPopular === true
              `${CollegesAPI}?districtId=${searchDistrict}&sort=rating${decodeURIComponent(
                pathAlter.replace(/[?]/g, "&")
              ).replace(/_/g, ",")}&page=${pageNum}`
            : router.query.collegelist
                .toLowerCase()
            ...
            ? // &&
              // sortPopular === true
              `${CollegesAPI}?districtId=${searchDistrict}&sort=rating&sortOrder=ASC${decodeURIComponent(
                pathAlter.replace(/[?]/g, "&")
              ).replace(/_/g, ",")}&page=${pageNum}`
            ...
            ? // &&
              // sortPopular === true
              `${CollegesAPI}?page=${pageNum}${decodeURIComponent(
                pathAlter.replace(/[?]/g, "&")
              ).replace(/_/g, ",")}`
            : menuname == "tamilnadu-colleges" && sortPopular === true
            ? // &&
              // sortPopular === true
              `${CollegesAPI}?page=${pageNum}&sort=raking${decodeURIComponent(
                pathAlter.replace(/[?]/g, "&")
              ).replace(/_/g, ",")}`
            : menuname == "tamilnadu-colleges" && sortRatingHigh === true
            ? // &&
              // sortPopular === true
              `${CollegesAPI}?page=${pageNum}&sort=rating${decodeURIComponent(
                pathAlter.replace(/[?]/g, "&")
              ).replace(/_/g, ",")}`
            : menuname == "tamilnadu-colleges" && sortRatingLow === true
            ? // &&
              // sortPopular === true
              `${CollegesAPI}?page=${pageNum}&sort=rating&sortOrder=ASC${decodeURIComponent(
                pathAlter.replace(/[?]/g, "&")
              ).replace(/_/g, ",")}`
  ...
  const [coursecheck, setCoursecheck] = useState(
    new Array(coursedatas.length).fill(false)
  );
  ...
  useEffect(() => {
    ...
    if (router.query.districtId) {
      var str = router.query.districtId.replace(/_/g, ",");
  ...
  // console.log(menuDID,router.query.collegelist && !router.query.course && activeDeg === true
  //   ? `${DistrictCityAPI}?streamId=${streamsid}&degreeId=${menuDID}${decodeURIComponent(
  //       pathAlter
  //     )
  //       .replace(/[?]/g, "&")
  ...
  useEffect(() => {
    ...
    if (clickfilter == true) {
      ...
      //   activeDeg,
      //   router.query.collegelist && !router.query.course && activeDeg === true
      //     ? `${DistrictCityAPI}?streamId=${streamsid}&degreeId=${degreeid}&districtId=${alldistrictinputsid}&cityId=${allcityinputsid}&courseId=${courseid}&collegetypeId=${collegetypeid}`
      //     : !router.query.course &&
      //       router.query.collegelist &&
      //       activeDeg === false &&
      //       activeStr === true
      //     ? `${DistrictCityAPI}?streamId=${streamsid}&districtId=${alldistrictinputsid}&cityId=${allcityinputsid}&courseId=${courseid}&collegetypeId=${collegetypeid}`
      //     : `${DistrictCityAPI}`
      // );
      try {
        fetch(
      ...
      mobilefilteroptions.degrees?.map((el, ind) => {
        if (el.streamid != streamsid) {
          setDegreeid([]);
        }
      });
    }
  ...
  const applyfilter = () => {
    ...
    // console.log(streamview, streamsid, streamname);
    if (streamview === "mobile" && streamsid.length > 0) {
    ...
    >
      ...
      >
        <GridContainer
          ...
          sx={{
            height: "56px",
            boxShadow: "0 0.5px 1px rgba(126,133,148,.15)",
            position: "fixed",
            zIndex: "999",
            alignItems: "center",
      ...
      <GridContainer container pl={3} height="100%">
        <FormControl>
          ...
          {[
            ...
            .slice(
              0,
              router.query.course && !router.query.courseId
                ? 5
                : router.query.courseId
                ? 3
                : 3
  ...
  return (
    <>
      ...
      <Stack
        width="100%"
        height={right === true ? "200px" : "100%"}
        justifyContent="center"
        alignItems="center"
        display={{ md: "none", xs: "flex" }}
        sx={{ overflow:right === true ?'hidden':'auto', mt:right === true ?2:0 }}
      >
        <Box
          sx={{
            overflowX: {
        ...
        <Typography sx={{ fontFamily: "Alata", mb: 1 }}>
          {totalcards} {totalcards > 1 ? "Colleges" : "College"} Available
        </Typography>
        {posts1.length === 0 ? (
          <>
            ...
            >
              <Divider
                ...
                sx={{
                  bgcolor: themeMode === true ? "common.white" : "common.black",
                  height: "1px",
                  width: "25%",
                }}
              />
        ...
        <GridContainer
          ...
          rowSpacing={{ xs: 1 }}
          // sx={{ overflow: right ===  true ? 'hidden' :'none' }}
        >
          {posts1.length != 0 ? (
            posts1
              .sort((a, b) => {
      ...
      {loading ? (
        ...
        >
          ...
          >
            ...
            {streamsid.length != 0 ||
            degreeid.length != 0 ||
            alldistrictinputsid.length != 0 ||
            allcityinputsid.length != 0 ||
            courseid.length != 0 ||
            collegetypeid.length != 0 ? (
              <GridContainer
                item
                lg={12}
              ...
              >
                <Typography sx={{ fontFamily: "Alata", mb: 2, ml: 3 }}>
                  {totalcards} {totalcards > 1 ? "Colleges" : "College"}{" "}
                  Available
                </Typography>
